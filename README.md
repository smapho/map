...
3. **Authentication** → Sign-in method で「メール/パスワード」を有効化
   - 管理画面（`admin/index.html`）から自分で「新規登録」できるので、Console側でユーザーを手動作成する必要はありません
...
5. Firestore の「ルール」タブを開き、[firestore.rules](firestore.rules) の内容に置き換えて公開
   ...
   - 書き込みは `firestore.rules` 内の許可リスト（`isAllowedAdmin()` 関数のメールアドレス配列）に載っているメールアドレスかつメール認証済みのアカウントのみ許可。**自分のメールアドレスをこの配列に入れておいてください**（既定値は `smapho1220@gmail.com`）。管理者を増やす場合はここにメールアドレスを追加してから、Console側の「ルール」タブに再度貼り付けて公開してください。
...
1. PCで `/admin/` を開く
   - 初回は「新規登録」からメールアドレス（`firestore.rules` の許可リストに入れたもの）とパスワードでアカウント作成 → 届いた確認メールのリンクをクリック → 管理画面で「確認できたら再読み込み」を押す
   - 2回目以降は同じメールアドレス・パスワードで「ログイン」
